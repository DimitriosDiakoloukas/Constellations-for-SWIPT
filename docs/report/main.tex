...
\end{itemize}
This volatility on the values of \(v(i)\) makes the construction of a formula for SER as a function of \(\eps\) extremely hard. Luckily, we can bypass this by calculating for each \(\eps\) the new \(\dmin '\) and the array of \(v(i)\)s. Of 
% We have devised a Monte-Carlo simulation environment where 
...
\begin{align}
    \text{SER}_i &= v(i)Q\left(\sqrt{\frac{6E_s }{(M^2 - 1) N_0}}\right) = v(i)Q\left(\frac{\dmin }{\sqrt{2N_0}}\right) \\
    \text{SER} &= \frac{1}{M }\sum_{i = 1}^{M } \text{SER}_i = \frac{1}{M }\sum_{i = 1}^{M }v(i)Q\left(\frac{\dmin }{\sqrt{2N_0}}\right)
\end{align}
For \(\eps = 0\), this becomes 
\begin{equation}
    \text{SER} = Q\left(\sqrt{\frac{6E_s }{(M^2 - 1) N_0}}\right) = Q\left(\sqrt{\frac{\log_2 M \cdot\text{SNR}}{(M^2 - 1)}}\right).
\end{equation}

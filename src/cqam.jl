...
function cqam_initialization2(M::Int=16,  N::Int=4; dmin=0.1)
    ...
    for i ∈ 3:N-1
        before = symbols[i-2][2]
        next_symbol = symbols[i-1][1] + symbols[i-1][2] - before
    ...
    if length(symbols) < N
        ...
        circle_last = psk_initialization(n; Es=r_last^2, offset_angle=offset_angle+π/(2n))
